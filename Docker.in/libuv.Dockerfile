...
RUN cd ${BUILDDIR} && \
    ...
    cmake -Bbuild -DCMAKE_BUILD_TYPE=Release -DLIBUV_BUILD_BENCH=OFF -DLIBUV_BUILD_TESTS=OFF -DCMAKE_TOOLCHAIN_FILE=${CROSS_ROOT}/Toolchain.cmake -DCMAKE_INSTALL_PREFIX=${CROSS_ROOT} && \
